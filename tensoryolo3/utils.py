...
def draw_box(image, bbox):
    ...
    new_bbox = tf.concat([tf.cast(ymin, tf.float32) / height, tf.cast(xmin, tf.float32) / weight,
                          tf.cast(ymax, tf.float32) / height, tf.cast(xmax, tf.float32) / weight], 2)
...
def voc_ap(rec, prec):
    ...
    rec.append(1.0)  # 在列表结尾插入1.0
    mrec = rec[:]
    prec.insert(0, 0.0)  # 在列表开头插入0.0
    prec.append(0.0)  # 在列表结尾插入1.0
    mpre = prec[:]
