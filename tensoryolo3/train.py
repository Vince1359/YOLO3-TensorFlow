...
from tensoryolo3.data_reader import Reader
from tensoryolo3 import config
from tensoryolo3.model import YOLO
from tensoryolo3.predict import YOLOPredictor
from tensoryolo3.utils import draw_box, load_weights, letterbox_image, voc_ap
...
def eval(model_path, min_iou=0.5, yolo_weights=None):
    ...
    for class_index, class_name in enumerate(sorted(gt_counter_per_class.keys())):
        ...
        sum_ap += ap
    coco_map = sum_ap / len(gt_counter_per_class) * 100
    print("The Model Eval coco_map: {}".format(coco_map))
