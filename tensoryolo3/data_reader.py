...
class Reader:
    def __init__(self, mode, data_dir, anchors_path, num_classes, tfrecord_num=12, input_shape=416, max_boxes=20):
        ...
        :param num_classes: 数据集图片类别数量
        :param tfrecord_num: 分成多少个tfrecord
        :param input_shape: 图像输入模型的大小
        :param max_boxes: 每张图片最大的box数量
        """
    ...
    def preprocess_true_boxes(self, true_boxes):
        ...
        :param true_boxes: ground truth box 形状为[boxes, 5], x_min, y_min, x_max, y_max, class_id
        :return:
        """
    ...
    def read_annotations(self):
        ...
        with open(self.annotations_file[self.mode], encoding='utf-8') as file:
            ...
            for ant in annotations:
                ...
                name = os.path.join(self.data_file[self.mode], '%012d.jpg' % image_id)
                cat = ant['category_id']
                if 1 <= cat <= 11:
                    cat = cat - 1
                elif 13 <= cat <= 25:
    ...
    def convert_to_tfrecord(self, tfrecord_path, num_tfrecords):
        ...
        :param tfrecord_path: tfrecord文件存储路径
        :param num_tfrecords: 分成多少个tfrecord
        :return:
        """
    ...
    def parser(self, serialized_example):
        """
        解析tfRecord数据
        :param serialized_example: 序列化的每条数据
        :return:
        """
    ...
    def preprocess(self, image, bbox):
        """
        对图片进行预处理，增强数据集
        :param image: tensorflow解析的图片
        :param bbox: 图片中对应的box坐标
        :return:
        """
        ...
        if self.mode == 'train':
            ...
            def _flip_left_right_boxes(boxes):
                xmin_i, ymin_i, xmax_i, ymax_i, label_i = tf.split(value=boxes, num_or_size_splits=5, axis=1)
                flipped_xmin = tf.subtract(input_width, xmax_i)
                flipped_xmax = tf.subtract(input_width, xmin_i)
                flipped_boxes = tf.concat([flipped_xmin, ymin_i, flipped_xmax, ymax_i, label_i], 1)
        ...
        bbox = tf.cond(tf.greater(tf.shape(bbox)[0], config.max_boxes), lambda: bbox[:config.max_boxes],
                       lambda: tf.pad(bbox, paddings=[[0, config.max_boxes - tf.shape(bbox)[0]], [0, 0]],
                                      mode='CONSTANT'))
        return image, bbox
    ...
    def build_dataset(self, batch_size):
        ...
        :return: dataset: 返回tensorflow的dataset
        """
