...
class YOLO:
    ...
    def _get_class(self):
        """
        获取类别名字
        :return: class_names: coco数据集类别对应的名字
        """
    ...
    def _batch_normalization_layer(input_layer, name=None, training=True, norm_decay=0.99, norm_epsilon=1e-3):
        ...
        :param input_layer: 输入的四维tensor
        :param name: batch_norm层的名字
    ...
    def _conv2d_layer(inputs, filters_num, kernel_size, name, use_bias=False, strides=1):
        ...
        比如，输入图片的大小为416*416，卷积核大小为3，若stride为2时，（416 - 3 + 2）/ 2 + 1，
        计算结果为208，相当于做了池化层处理
    ...
    def _darknet53(self, inputs, conv_index, training=True, norm_decay=0.99, norm_epsilon=1e-3):
        ...
        :return: route2: 返回第43层卷积计算结果26x26x512, 供后续使用
        :return: conv: 经过52层卷积计算之后的结果, 输入图片为416x416x3，则此时输出的结果shape为13x13x1024
        :return: conv_index: 卷积层计数，方便在加载预训练模型时使用
        """
    ...
    def _yolo_block(self, inputs, filters_num, out_filters, conv_index, training=True, norm_decay=0.99,
        ...
        :return: conv: 返回最后一层卷积的结果
        :return: conv_index: conv层计数
        """
    ...
    def yolo_head(feats, anchors, num_classes, input_shape, training=True):
        ...
        :param feats: 输入的特征feature map
        :param anchors: 针对不同大小的feature map的anchor
        :param num_classes: 类别的数量
        :param input_shape: 图像的输入大小，一般为416
        :param training: 是否训练，用来控制返回不同的值
        :return:
        """
    ...
    def yolo_boxes_scores(self, feats, anchors, num_classes, input_shape, image_shape):
        ...
        :param feats: 模型输出feature map
        :param anchors: 模型anchors
        :param num_classes: 数据集类别数
        :param input_shape: 训练输入图片大小
        :param image_shape: 原始图片的大小
        :return:
        """
    ...
    def box_iou(box1, box2):
        """
        计算box tensor之间的iou
        :param box1: shape=[grid_size, grid_size, anchors, xywh]
        :param box2: shape=[box_num, xywh]
        :return: iou:
        """
    ...
    def yolo_loss(self, yolo_output, y_true, anchors, num_classes, ignore_thresh=.5):
        """
        yolo模型的损失函数
        :param yolo_output: yolo模型的输出
        :param y_true: 经过预处理的真实标签，shape为[batch, grid_size, grid_size, 5 + num_classes]
        :param anchors: yolo模型对应的anchors
        :param num_classes: 类别数量
        :param ignore_thresh: 小于该阈值的box我们认为没有物体
        :return: loss: 每个batch的平均损失值
        """
        loss = 0
        ...
        for index in range(3):
            ...
            mask = tf.TensorArray(dtype=tf.float32, size=1, dynamic_size=True)
